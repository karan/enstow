...
# Valkey/Redis
VALKEY_HOST = "valkey-test-e2e"
VALKEY_PASSWORD = os.getenv("VALKEY_PASSWORD", "test_valkey_password")
VALKEY_EXPECTED_KEYS = {
    "test:key1": "Redis Value 1",
    "test:key2": "Redis Value 2"
}
VALKEY_EXPECTED_LIST = ["item3", "item2", "item1"] # LPUSH reverses order
VALKEY_EXPECTED_HASH = {"field1": "value1", "field2": "value2"}
...
def verify_valkey_redis_restore():
    verify_log("Verifying Valkey/Redis restore...")
    try:
        r = redis.Redis(host=VALKEY_HOST, port=6379, password=VALKEY_PASSWORD, db=0)
        
        # Verify simple keys
        for key, expected_value in VALKEY_EXPECTED_KEYS.items():
            value = r.get(key)
            if value is None or value.decode('utf-8') != expected_value:
                verify_log(f"Valkey/Redis key '{key}' verification failed. Expected '{expected_value}', got '{value.decode('utf-8') if value else None}'", level="FAIL")
        
        # Verify list
        fetched_list = [item.decode('utf-8') for item in r.lrange("test:list", 0, -1)]
        if len(fetched_list) != len(VALKEY_EXPECTED_LIST) or sorted(fetched_list) != sorted(VALKEY_EXPECTED_LIST):
            verify_log(f"Valkey/Redis list 'test:list' verification failed. Expected {VALKEY_EXPECTED_LIST}, got {fetched_list}", level="FAIL")

        # Verify hash
        fetched_hash = {k.decode('utf-8'): v.decode('utf-8') for k, v in r.hgetall("test:hash").items()}
        if len(fetched_hash) != len(VALKEY_EXPECTED_HASH) or fetched_hash != VALKEY_EXPECTED_HASH:
            verify_log(f"Valkey/Redis hash 'test:hash' verification failed. Expected {VALKEY_EXPECTED_HASH}, got {fetched_hash}", level="FAIL")

        verify_log("Valkey/Redis restore verified successfully.")
    except Exception as e:
        verify_log(f"Valkey/Redis verification failed: {e}", level="FAIL")
